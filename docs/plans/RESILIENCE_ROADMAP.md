...
Waymark is a Capacitor 8 hybrid app (React 19 + Vite 7 + Tailwind 4) running inside a WKWebView on iOS. The app is architecturally sound and iPhone-first by design, but has gaps in failure handling, loading UX, and observability that need to be closed before relying on it daily.
...
Ordered by risk (lowest first) and impact (highest first). Each step is independent and can be built, tested, and committed on its own.
...
#### Step 7 - View transitions activation
...
2. Work through steps 1-8 in order
3. Each step can be built, tested on Simulator, and committed independently
4. Steps 1-5 are the priority (safety net + observability)
5. Steps 6-8 are polish and systemic improvement
6. Test on Simulator after each step before moving to the next
